...
经过这一步骤之后，数组 p 到 r 之间的数据就被分成了三个部分，前面 p 到 q-1 之间都是小于 pivot 的，中间是 pivot，后面的 q+1 到 r 之间是大于 pivot 的。也就是说，如果 pivot 的索引是 index，那 **pivot 就是当前数组中的第 index + 1 大元素**。
...
- 通过游标 i 把 A[p...r-1]分成两部分：
  - 已处理区间 A[p...i-1]，元素都小于 pivot
  - 未处理区间 A[i...r-1]
- 每次都从未处理的区间 A[i...r-1]中取一个元素 A[j]，与 pivot 对比，如果小于 pivot，则将其加入到已处理区间的尾部，也就是 A[i]的位置。

#### 交换

**这里有一个技巧 —— 交换，帮助我们在 O(1) 的时间复杂度内完成插入操作**。在数组某个位置插入元素，需要搬移数据，非常耗时。但是将 A[i] 与 A[j] 交换，就可以在 O(1) 时间复杂度内将 A[j] 放到下标为 i 的位置。
...
如果每次分区操作，都能正好把数组分成大小接近相等的两个小区间，那快排的时间复杂度递推求解公式跟归并是相同的，所以快排的时间复杂度也是 O(n * logn)。
...
这个公式的递推求解的过程非常复杂，所以直接给你结论：T(n) 在大部分情况下的时间复杂度都可以做到 O(nlogn)，只有在极端情况下，才会退化到 O(n^2)。
...
借助选择排序思想，每次取数组中的最小值，将其移动到数组的最前面，然后在剩下的数组中继续找最小值，以此类推，执行 K 次，找到的数据就是第 K 大元素。

```ts
function findTopK(arr: number[], k: number) {
  const selectionOperator = (arr: number[], start: number) => {
    let index = start
...
当 K 是比较小的常量时，比如 1、2 那时间复杂度确实是 O(n)；但当 K 等于 n/2 或者 n 时，这种情况下的时间复杂度就是 O(n ^ 2) 了。所以 O(k * n) 不能简单地和 O(n) 划等号。
...
- 对数组 A 原地分区，这样数组就分成了三部分 A[0...p-1], A[p], A[p+1...n-1]。其中 n 为数组长度
  - 如果 p = K - 1，那 A[p] 就是要求解的元素
  - 如果 p < K - 1, 说明第 K 大元素出现在 A[p+1...n-1]区间，我们再按照上面的思路递归地在 A[p+1...n-1]这个区间内查找
  - 如果 p > K - 1，那我们就在 A[0...p-1]区间查找
...
function findTopK(arr: number[], k: number) {
  ...
  let q = partition(arr, 0, arr.length - 1)
  while (q !== k - 1) {
    if (q > k - 1) {
